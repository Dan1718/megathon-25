...
// =================================================================================
// --- Operational Transform Core Data Structures ---
// =================================================================================
...
// =================================================================================
// --- GTK Application Data Structures ---
// =================================================================================
...
// Data structure for tag removal
typedef struct {
...
// =================================================================================
// --- Operational Transform Core Logic ---
// =================================================================================
Operation server_receive_op(Server* server, Operation op_from_client) {
...
void client_receive_op(Client* client, Operation op_from_server) {
    ...
    if (client->pending_op && op_from_server.client_id == client->id) {
        // CASE 1: This is an ACK for our own pending operation.
        // The operation has already been applied locally, so we MUST NOT apply it again.
        // We just need to clear the pending state.
        g_print("   [Client %d] -> It's an ACK. Clearing pending state.\n", client->id);
        free(client->pending_op);
        client->pending_op = NULL;

    } else {
        // CASE 2: This is a new operation from another client.
        // We need to apply it to our document.
        if (client->pending_op) {
            // If we have a pending operation, we must transform both the incoming
            // operation and our own pending operation to ensure convergence.
        ...
        // Apply the (potentially transformed) remote operation to our internal document.
        apply_op_to_doc(client->doc, &op_to_apply);
    }
    
    // The revision number is always incremented after processing a valid server operation.
    client->revision++;
...
// =================================================================================
// --- Operational Transform Utility Functions ---
// =================================================================================
...
Document* create_document(const char* initial_text) {
    ...
    doc->capacity = len + 16; 
    doc->text = malloc(doc->capacity);
    strcpy(doc->text, initial_text); 
    doc->size = len;
    return doc;
}
...
// =================================================================================
// --- GTK Application Helper Functions ---
// =================================================================================
